...
### 2. 翻訳品質基準
✅ **必須**:
- **{LANGUAGE_NATIVE}にピッタリの単語があればそれを使う**
- 単語だけでもOK、単語+補足説明でもOK
- **名詞としての意味**を説明（動詞・形容詞の意味は禁止）
- {LANGUAGE_NATIVE}で自然で正確な表現
- 同一言語内での重複語を排除

**重要**: 
- 英語の `meaning_en` は長い説明文だが、これは英語が単語そのものを使えないため
- {LANGUAGE_NATIVE}では、**適切な単語があればそれを使うべき**
...
処理:
  ...
  ステップ2: {LANGUAGE_NATIVE}に「修道院」のような適切な単語があるか確認
...
処理:
  ...
  ステップ2: {LANGUAGE_NATIVE}で最も自然な表現を選ぶ
    → ピッタリの単語があればそれを使う
    → 補足説明を追加してもOK
...
処理:
  ...
  ステップ2: {LANGUAGE_NATIVE}に適切な単語があるか確認
    → 「要約」「抄録」「アブストラクト」など
...
✅ 同一言語内に重複語がない  
✅ タイプミス・アクセント記号の誤りゼロ  
✅ 自然で正確な{LANGUAGE_NATIVE}（英語の長い説明を無理に模倣していない）
...
3. ✅ 重複チェック: エラー0件
4. ✅ コミット完了
5. ✅ 進捗ファイル更新: `.claude/workflow/progress-stage4-{LANGUAGE}.txt`
