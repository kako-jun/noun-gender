# Stage 3: ヒンディー語性別翻訳タスク

## あなたの役割
あなたは**ヒンディー語の専門翻訳者**です。`data/word_gender_translations.csv`の**hi列（hi_translation, hi_gender）のみ**を翻訳してください。
...
for i, row in enumerate(rows, start=1):
    ...
    # ヒンディー語に翻訳（必ず名詞として）
    # 実際の翻訳処理を実行
    translation = translate_to_hi(en, first_meaning)
    gender = get_hi_gender(translation)
    
    # 上書き
    row['hi_translation'] = translation
    row['hi_gender'] = gender
...
echo "stage3-hi: 4592/4592 (100%)" > .claude/workflow/progress-stage3-hi.txt
echo "完了日時: $(date)" >> .claude/workflow/progress-stage3-hi.txt
```

### ステップ4: 変更をコミット
```bash
git add data/word_gender_translations.csv .claude/workflow/progress-stage3-hi.txt
git commit -m "feat(stage3-hi): complete ヒンディー語 gender translations for all 4592 words"
```
...
✅ 4,592行すべての`hi_translation`, `hi_gender`が記入されている  
✅ すべて名詞として翻訳されている  
✅ セミコロンルール100%遵守  
✅ 性別記号がすべて正しい  
✅ タイプミス・アクセント記号の誤りゼロ  
