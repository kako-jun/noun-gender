...
## あなたの役割
あなたは**スペイン語の専門翻訳者**です。`data/word_meaning_translations.csv`の**meaning_es列のみ**を翻訳してください。
...
### 1. 翻訳対象
- `meaning_en`列の**全体**をスペイン語に翻訳
- セミコロンの数は一致不要（適切な要約OK）

### 2. 翻訳品質基準
✅ **必須**:
- **スペイン語にピッタリの単語があればそれを使う**
- 単語だけでもOK、単語+補足説明でもOK
...
- スペイン語では、**適切な単語があればそれを使うべき**
- 例: "修道院" > "僧院が使用する建物"（長い説明より単語を優先）

### 3. 重複語の排除
❌ 悪い例:
```
meaning_es: "能力; 能力; 技能"  ← 「能力」が2回
```

✅ 良い例:
```
meaning_es: "能力; 技能"  ← 重複削除
```
...
for i, row in enumerate(rows, start=1):
    ...
    # meaning_enの全体をスペイン語に翻訳
    translation = translate_meaning_to_es(meaning_en)
...
echo "stage4-es: 4592/4592 (100%)" > .claude/workflow/progress-stage4-es.txt
echo "完了日時: $(date)" >> .claude/workflow/progress-stage4-es.txt
```

### ステップ4: 変更をコミット
```bash
git add data/word_meaning_translations.csv .claude/workflow/progress-stage4-es.txt
git commit -m "feat(stage4-es): complete スペイン語 meaning translations for all 4592 words"
```
...
処理:
  ...
  ステップ2: スペイン語に「修道院」のような適切な単語があるか確認
...
✅ 4,592行すべての`meaning_es`が記入されている  
✅ **スペイン語に適切な単語があればそれを使っている**  
✅ すべて名詞の意味として翻訳されている（動詞・形容詞は禁止）  
✅ 同一言語内に重複語がない  
✅ タイプミス・アクセント記号の誤りゼロ  
✅ 自然で正確なスペイン語（英語の長い説明を無理に模倣していない）
...
3. ✅ 重複チェック: エラー0件
4. ✅ コミット完了
5. ✅ 進捗ファイル更新: `.claude/workflow/progress-stage4-es.txt`

---

**言語**: スペイン語  
**担当列**: meaning_es  
**品質基準**: 自然なスペイン語表現、名詞の意味、重複排除  
