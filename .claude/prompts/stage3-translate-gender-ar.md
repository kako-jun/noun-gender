...
処理:
  ...
  ステップ3: "Lack"をアラビア語の名詞に翻訳
    → ar_translation: "manque" (フランス語の場合)
    → ar_gender: "m"

正解:
  ar_translation: "manque"
  ar_gender: "m"

不正解（これらは絶対にダメ）:
  ❌ ar_translation: "manque et absence" (複数の意味を含めた)
  ❌ ar_translation: "absence" (2番目の意味を使った)
  ❌ ar_translation: "le fait de manquer" (説明文にした)
```
...
正解:
  ar_translation: "abbaye" (単数形)
  ar_gender: "f"

不正解:
  ❌ ar_translation: "abbayes" (複数形にした)
  ❌ ar_translation: "bâtiment monastique" (説明的にした)
```
...
処理:
  ...
  ステップ2: "Summary"をアラビア語の**名詞**に翻訳
    → ❌ "abstrait" (形容詞)
    → ✅ "résumé" (名詞)

正解:
  ar_translation: "résumé" (名詞「要約」)
  ar_gender: "m"

不正解:
  ❌ ar_translation: "abstrait" (形容詞で翻訳した)
  ❌ ar_translation: "résumer" (動詞で翻訳した)
```
...
正解:
  ar_translation: "capacité" (名詞1語)
  ar_gender: "f"

不正解:
  ❌ ar_translation: "possession de moyens" (説明的すぎる)
  ❌ ar_translation: "pouvoir" (動詞的な意味)
```
...
4. ✅ 3文字未満の翻訳: 0件
5. ✅ コミット完了
6. ✅ 進捗ファイル更新: `.claude/workflow/progress-stage3-ar.txt`
