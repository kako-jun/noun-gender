...
## あなたの役割
あなたは**ロシア語の専門翻訳者**です。`data/word_gender_translations.csv`の**ru列（ru_translation, ru_gender）のみ**を翻訳してください。
...
処理:
  ...
  ステップ3: "Lack"をロシア語の名詞に翻訳
    → ru_translation: "manque" (フランス語の場合)
    → ru_gender: "m"

正解:
  ru_translation: "manque"
  ru_gender: "m"

不正解（これらは絶対にダメ）:
  ❌ ru_translation: "manque et absence" (複数の意味を含めた)
  ❌ ru_translation: "absence" (2番目の意味を使った)
  ❌ ru_translation: "le fait de manquer" (説明文にした)
```
...
正解:
  ru_translation: "abbaye" (単数形)
  ru_gender: "f"

不正解:
  ❌ ru_translation: "abbayes" (複数形にした)
  ❌ ru_translation: "bâtiment monastique" (説明的にした)
```
...
処理:
  ...
  ステップ2: "Summary"をロシア語の**名詞**に翻訳
    → ❌ "abstrait" (形容詞)
    → ✅ "résumé" (名詞)

正解:
  ru_translation: "résumé" (名詞「要約」)
  ru_gender: "m"

不正解:
  ❌ ru_translation: "abstrait" (形容詞で翻訳した)
  ❌ ru_translation: "résumer" (動詞で翻訳した)
```
...
正解:
  ru_translation: "capacité" (名詞1語)
  ru_gender: "f"

不正解:
  ❌ ru_translation: "possession de moyens" (説明的すぎる)
  ❌ ru_translation: "pouvoir" (動詞的な意味)
```
...
for i, row in enumerate(rows, start=1):
    ...
    # ロシア語に翻訳（必ず名詞として）
    # 実際の翻訳処理を実行
    translation = translate_to_ru(en, first_meaning)
    gender = get_ru_gender(translation)
    
    # 上書き
    row['ru_translation'] = translation
    row['ru_gender'] = gender
...
echo "stage3-ru: 4592/4592 (100%)" > .claude/workflow/progress-stage3-ru.txt
echo "完了日時: $(date)" >> .claude/workflow/progress-stage3-ru.txt
```

### ステップ4: 変更をコミット
```bash
git add data/word_gender_translations.csv .claude/workflow/progress-stage3-ru.txt
git commit -m "feat(stage3-ru): complete ロシア語 gender translations for all 4592 words"
```
...
4. ✅ **性別チェック**: 性別記号が m (男性名詞), f (女性名詞), n (中性名詞) の範囲内か
...
✅ 4,592行すべての`ru_translation`, `ru_gender`が記入されている  
✅ すべて名詞として翻訳されている  
✅ セミコロンルール100%遵守  
✅ 性別記号がすべて正しい  
✅ タイプミス・アクセント記号の誤りゼロ  
...
4. ✅ 3文字未満の翻訳: 0件
5. ✅ コミット完了
6. ✅ 進捗ファイル更新: `.claude/workflow/progress-stage3-ru.txt`
