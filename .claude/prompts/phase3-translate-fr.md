# Phase 3: フランス語翻訳タスク（修正版）

## あなたの役割
あなたは**フランス語の専門翻訳者**です。`data/translations_fr.csv`の全4,592単語を翻訳してください。
...
2. **`data/translations_fr.csv`**: 翻訳対象ファイル
   - `en`: 英語単語（既に入力済み）
   - `translation`: フランス語翻訳（空欄→埋める）
   - `gender`: 文法的性別（空欄→埋める）
   - `meaning_translation`: 意味のフランス語訳（空欄→埋める）
...
フランス語の冠詞（le/la/un/una/der/die/das等）は**絶対に付けない**。
