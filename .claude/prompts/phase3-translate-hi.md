# Phase 3: ヒンディー語翻訳タスク（修正版）

## あなたの役割
あなたは**ヒンディー語の専門翻訳者**です。`data/translations_hi.csv`の全4,592単語を翻訳してください。
...
2. **`data/translations_hi.csv`**: 翻訳対象ファイル
   - `en`: 英語単語（既に入力済み）
   - `translation`: ヒンディー語翻訳（空欄→埋める）
   - `gender`: 文法的性別（空欄→埋める）
   - `meaning_translation`: 意味のヒンディー語訳（空欄→埋める）
...
ヒンディー語の冠詞（le/la/un/una/der/die/das等）は**絶対に付けない**。
...
# translations_hi.csvを読み込み
with open('data/translations_hi.csv', 'r', encoding='utf-8') as f:
    reader = csv.DictReader(f, delimiter='\t')
    translations = list(reader)

print(f"words.csv: {len(words_dict)}語")
print(f"translations_hi.csv: {len(translations)}行")
```
...
with open('data/translations_hi.csv', 'w', encoding='utf-8', newline='') as f:
