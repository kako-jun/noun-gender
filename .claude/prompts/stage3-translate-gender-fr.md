...
## あなたの役割
あなたは**フランス語の専門翻訳者**です。`data/word_gender_translations.csv`の**fr列（fr_translation, fr_gender）のみ**を翻訳してください。
...
処理:
  ...
  ステップ3: "Lack"をフランス語の名詞に翻訳
    → fr_translation: "manque" (フランス語の場合)
    → fr_gender: "m"
...
不正解（これらは絶対にダメ）:
  ❌ fr_translation: "manque et absence" (複数の意味を含めた)
  ❌ fr_translation: "absence" (2番目の意味を使った)
  ❌ fr_translation: "le fait de manquer" (説明文にした)
```
...
不正解:
  ❌ fr_translation: "abbayes" (複数形にした)
  ❌ fr_translation: "bâtiment monastique" (説明的にした)
```
...
不正解:
  ❌ fr_translation: "abstrait" (形容詞で翻訳した)
  ❌ fr_translation: "résumer" (動詞で翻訳した)
```
...
正解:
  fr_translation: "capacité" (名詞1語)
  fr_gender: "f"

不正解:
  ❌ fr_translation: "possession de moyens" (説明的すぎる)
  ❌ fr_translation: "pouvoir" (動詞的な意味)
```
...
for i, row in enumerate(rows, start=1):
    ...
    # フランス語に翻訳（必ず名詞として）
    # 実際の翻訳処理を実行
    translation = translate_to_fr(en, first_meaning)
    gender = get_fr_gender(translation)
...
### ステップ4: 変更をコミット
```bash
git add data/word_gender_translations.csv .claude/workflow/progress-stage3-fr.txt
git commit -m "feat(stage3-fr): complete フランス語 gender translations for all 4592 words"
```
...
4. ✅ **性別チェック**: 性別記号が m (男性名詞: le), f (女性名詞: la) の範囲内か
...
4. ✅ 3文字未満の翻訳: 0件
5. ✅ コミット完了
6. ✅ 進捗ファイル更新: `.claude/workflow/progress-stage3-fr.txt`
